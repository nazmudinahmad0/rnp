<?php
    ...
    if (isset($_POST['edit_kelas'])) {
        ...
        if ($send AND $_SERVER["REQUEST_METHOD"] == "POST") {
            ...
            EmailMKT='$EmailMKT'
            where NumDoc='$NumDoc'";
            $historyfa = "INSERT INTO thistorycomment (NumDoc,Comment,DateComment,Dept) VALUES ('$NumDoc','$comment','$DateCosting','$Dept')";
        
            //Menyimpan ke tabel kelas
            $simpan_kelas=mysqli_query($kon,$sql);
            $simpan_history=mysqli_query($kon,$historyfa);

            if ($simpan_kelas AND $simpan_history) {
...
<?php 
    ...
    //$a = isset($data['a']) ? $data['a'] : '';
    $tanggal_request = $b;
    $tgl1 = date("d M Y H:i:s", strtotime($tanggal_request)); 
    $tanggal_feedback = $i;
    $tgl2 = date("d M Y H:i:s", strtotime($tanggal_feedback));
    $tanggal_target = $q;
    $tgl3 = date("d M Y H:i:s", strtotime($tanggal_target));
    $tanggal_verifikasi = $s;
    $tgl4 = date("d M Y H:i:s", strtotime($tanggal_verifikasi));
    $tanggal_estimasi = $t;
    $tgl5 = date("d M Y H:i:s", strtotime($tanggal_estimasi));
?>
...
<div class="row">
    ...
    <div class="row">
        <div class="col-sm-12">
            <div class="form-group">
                ...
                <input type="text" name="DateCosting" value="<?php date_default_timezone_set('Asia/Jakarta'); echo date("Y-m-d H:i:s");?>" class="form-control" readonly>
            </div>
        </div>
    </div>
    <div class="row">
    ...
    <form method="post" action="pages/fa/beranda/send.php" enctype="multipart/form-data">
    <div class="row">
        <div class="col-sm-12">
            <div class="form-group">
                <input type="hidden" name="subjek" value="Return From FA" class="form-control">
                <input type="hidden" name="NumDoc" value="<?php echo $a;?>" class="form-control" readonly>
                <input type="hidden" name="StatusReturn" value="NPD" class="form-control">
                <input type="hidden" name="StatusDoc" value="OPEN" class="form-control">
                <input type="hidden" name="EmailRND" value="<?php echo $x;?>" class="form-control" readonly>
                <input type="hidden" name="CodeProject" value="<?php echo $data['e'];?>" class="form-control" required>
