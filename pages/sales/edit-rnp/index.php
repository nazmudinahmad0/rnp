    ...
    if(empty($attachment) AND empty($attachment1)){
        ...
        if ($send AND $_SERVER["REQUEST_METHOD"] == "POST") {
           ...
           // $CodeProject=$_POST["CodeProject"];
            ...
            $GenifName=$_POST["GenifName"];
            $apn=implode(",",$AppName);
            $frn=implode(",",$FrameName);
            $gen=implode(",",$GenifName);
            ...
            $sql2 = "INSERT INTO thistorycomment(NumDoc,DateComment,Dept,Comment) VALUES ('$NumDoc','$DR','$departemen','$comment')";

            if ($simpan_kelas AND $kon->query($sql2)) {
...
<?php 
    ...
    while($data = mysqli_fetch_array($hasil)){
        ...
        //$a = isset($data['a']) ? $data['a'] : '';
        $tanggal_request = $b;
        $tgl1 = date("Y-m-d H:i:s", strtotime($tanggal_request)); 
        $tanggal_feedback = $i;
        $tgl2 = date("Y-m-d H:i:s", strtotime($tanggal_feedback));
        $tanggal_target = $q;
        $tgl3 = date("Y-m-d H:i:s", strtotime($tanggal_target));
        $tanggal_verifikasi = $s;
        $tgl4 = date("Y-m-d H:i:s", strtotime($tanggal_verifikasi));
        $tanggal_update = $ut;
        $tgl5 = date("Y-m-d H:i:s", strtotime($tanggal_update));
        //$datahobi=explode(',', $data['ab']);
        /* $abc=$data['ab'];
        $datahobi=explode(",", "$abc");
        $def=$data['ac'];
        $dataframe=explode(",", "$def");
        $ghi=$data['ad'];
        $datageneralinf=explode(",", "$ghi"); */
         //$datahobi=explode(',', $data['ab']);
        $datahobi= explode(', ', $data['ab']);
...
<form action="pages/sales/edit-rnp/index.php" method="post" enctype="multipart/form-data">
    <div class="row">
        <div class="col-sm-3">
            <div class="form-group">
                ...
                <input type="hidden" name="subjek" value="Sales Correction BOM + Costing" class="form-control">
    ...
    <div class="row">
        <div class="col-sm-5">
            <div class="form-group">
                ...
                <!-- <select name="NPDType" id="NPDType" class="form-control">  
             ...
             <div class="row">
                <div class="col-sm-4">
                    <div class="form-group">
                        ...
                        <p><input type="checkbox" value="Roofing Outdoor " name="AppName[]" <?php 
                            if (in_array("Roofing Outdoor", $datahobi)) { 
                            echo "checked";
                            }
                            ?> /> Roofing Outdoor</p>
                        <p><input type="checkbox" value="Roofing Indoor " name="AppName[]"<?php if (in_array("Roofing Indoor", $datahobi)) {echo "checked";}?>/> Roofing Indoor</p>
                        <p><input type="checkbox" value="Surface Loose (outdoor)" name="AppName[]" <?php if (in_array("Surface Loose (outdoor)", $datahobi)) {echo "checked";}?>/> Surface Loose (outdoor)</p>
                        <p><input type="checkbox" value="Surface Loose (indoor)" name="AppName[]" <?php if (in_array("Surface Loose (indoor)", $datahobi)) {echo "checked";}?>/> Surface Loose (indoor)</p>
                        <p><input type="checkbox" value="Surface + backing (indoor)" name="AppName[]" <?php if (in_array("Surface + backing (indoor)", $datahobi)) {echo "checked";}?>/> Surface + backing (indoor)</p>
                        <p><input type="checkbox" value="Surface + frame (outdoor)" name="AppName[]"<?php if (in_array("Surface + frame (outdoor)", $datahobi)) {echo "checked";}?>/> Surface + frame (outdoor)</p>
                        <p><input type="checkbox" value="Surface + frame (indoor)" name="AppName[]" <?php if (in_array("Surface + frame (indoor)", $datahobi)) {echo "checked";}?>/> Surface + frame (indoor)</p>
                        <p><input type="checkbox" value="Handicraft (outdoor)" name="AppName[]"<?php if (in_array("Handicraft (outdoor)", $datahobi)) {echo "checked";}?>/> Handicraft (outdoor)</p>
                    </div>
                </div>
                <div class="col-sm-4">
                    <div class="form-group">
    ...
    <div class="row">
        <div class="col-sm-6">
            <div class="form-group">
                <label>Weaving Pattern <font color="red">* </font>:</label>
    ...
    <div class="row">
        <div class="col-sm-6">
            <div class="form-group">
                <label>What Fiber <font color="red">* </font>:</label>
                <input type="text" name="WhatFiber" value="<?php echo $wf;?>" class="form-control">
            </div>
        </div>
    </div>
    <div class="row">
    ...
    <div class="row">
        <div class="col-sm-3">
            <div class="form-group">
                ...
                <input type="hidden" name="DateVerification" value="<?php date_default_timezone_set('Asia/Jakarta'); echo date("Y-m-d");?>" class="form-control">
